import { useMemo, useRef } from 'react'
import { useFrame } from '@react-three/fiber'
import { AdditiveBlending } from 'three'
import type { Group, Mesh, MeshBasicMaterial, MeshStandardMaterial } from 'three'
import { useGameStore } from '@/game/store'
import { getProcTextureKit } from '@/game/proc'
import { ZoneLabel } from '@/game/ZoneLabel'
import { GAMMA_BRIDGE, GAMMA_CENTER, GAMMA_RADIUS, PAD_TOP } from '@/game/world'

const CYAN = '#3dd6c6'
const AMBER = '#f0a830'
const VIOLET = '#b48cff'
const STEEL = '#1a3344'

/** Splitter sits west of the spawn→terminal→gate walk line, angled back toward the pad core. */
const SPLITTER_POS: [number, number, number] = [-2.4, PAD_TOP, -1.6]
const SPLITTER_YAW = -2.1

/**
 * Wave 21 — Lesson 3 mastery unlocks as real range props.
 * Charge splitter: auto-present Y-splitter on the Alpha pad (auto-present over
 * placeable, same call as the wave-16 rails). Zone Gamma Relay: walkable
 * hexagonal relay pad west of Beta, violet accents so L3 reads distinct from
 * the cyan L2 annex. Expert rank insignia lives on the Player mesh itself.
 * Budget: ~34 small meshes, proc textures only, zero new point lights.
 */
export function L3UnlockProps() {
  return (
    <group>
      <SplitterProp />
      <GammaRelay />
    </group>
  )
}

const POP_STAGGER = 0.085
const POP_DURATION = 0.6
const FLASH_DURATION = 1.45

/** Overshoot ease-out — segments pop past full scale, then settle. */
function easeOutBack(p: number): number {
  const c1 = 1.70158
  const c3 = c1 + 1
  const q = p - 1
  return 1 + c3 * q * q * q + c1 * q * q
}

/** bp.relay.splitter — Y-splitter: one cyan trunk feeds two amber branch lines. */
function SplitterProp() {
  const unlocked = useGameStore((s) => s.hasRelaySplitter)
  const barMats = useRef<(MeshStandardMaterial | null)[]>([])
  const segRefs = useRef<(Group | null)[]>([])
  const ringRef = useRef<Mesh>(null)
  const ringMat = useRef<MeshBasicMaterial>(null)
  // One-shot materialization FX — fires on a live unlock transition only,
  // deferred until explore so the lesson overlay doesn't eat the beat
  // (same idiom as PadRailProp; HUD flash owns the audio blip).
  const prev = useRef<boolean | null>(null)
  const pending = useRef(false)
  const start = useRef(-1)

  useFrame((state) => {
    const t = state.clock.elapsedTime
    const mode = useGameStore.getState().mode

    if (prev.current === null) {
      prev.current = unlocked
    } else if (unlocked !== prev.current) {
      prev.current = unlocked
      if (unlocked) {
        if (mode === 'lesson') pending.current = true
        else start.current = t
      }
    }
    if (pending.current && mode !== 'lesson' && unlocked) {
      pending.current = false
      start.current = t
    }

    let firing = start.current >= 0
    const popT = firing ? t - start.current : 0
    if (firing && popT > FLASH_DURATION) {
      start.current = -1
      firing = false
    }
    const flash = firing ? Math.max(0, 1 - popT / FLASH_DURATION) : 0

    for (let i = 0; i < barMats.current.length; i++) {
      const m = barMats.current[i]
      if (m) m.emissiveIntensity = 0.85 + Math.sin(t * 2.2 - i * 0.7) * 0.3 + flash * 2.4
    }

    for (let i = 0; i < segRefs.current.length; i++) {
      const g = segRefs.current[i]
      if (!g) continue
      if (!firing) {
        if (g.scale.x !== 1) g.scale.setScalar(1)
        continue
      }
      const lp = (popT - i * POP_STAGGER) / POP_DURATION
      if (lp <= 0) g.scale.setScalar(0.001)
      else if (lp >= 1) g.scale.setScalar(1)
      else g.scale.setScalar(Math.max(0.001, easeOutBack(lp)))
    }

    // Additive ring sweeps out to the branch line as the emissive flash decays
    if (ringRef.current && ringMat.current) {
      const show = firing && flash > 0
      ringRef.current.visible = show
      if (show) {
        const rp = Math.min(popT / FLASH_DURATION, 1)
        ringRef.current.scale.setScalar(0.12 + rp * 1.13)
        ringMat.current.opacity = 0.7 * flash * flash
      }
    }
  })

  if (!unlocked) return null

  return (
    <group position={SPLITTER_POS} rotation={[0, SPLITTER_YAW, 0]}>
      {/* Trunk: feed post + cyan charge bar running into the hub */}
      <group
        ref={(g) => {
          segRefs.current[0] = g
        }}
      >
        <mesh position={[0, 0.31, 0.62]}>
          <cylinderGeometry args={[0.035, 0.05, 0.62, 6]} />
          <meshStandardMaterial color={STEEL} metalness={0.55} roughness={0.45} />
        </mesh>
        <mesh position={[0, 0.65, 0.31]}>
          <boxGeometry args={[0.07, 0.055, 0.7]} />
          <meshStandardMaterial
            ref={(m) => {
              barMats.current[0] = m
            }}
            color={CYAN}
            emissive={CYAN}
            emissiveIntensity={0.9}
            metalness={0.3}
            roughness={0.4}
          />
        </mesh>
      </group>
      {/* Hub: the split point */}
      <group
        ref={(g) => {
          segRefs.current[1] = g
        }}
      >
        <mesh position={[0, 0.68, 0]}>
          <octahedronGeometry args={[0.09, 0]} />
          <meshStandardMaterial color={CYAN} emissive={CYAN} emissiveIntensity={1.5} metalness={0.3} roughness={0.4} />
        </mesh>
      </group>
      {/* Branch arms: post + amber bar + device tip, mirrored across the trunk */}
      {([[-1, 0.94], [1, -0.94]] as const).map(([side, yaw], i) => (
        <group
          key={side}
          ref={(g) => {
            segRefs.current[2 + i] = g
          }}
        >
          <mesh position={[side * 0.58, 0.31, -0.42]}>
            <cylinderGeometry args={[0.035, 0.05, 0.62, 6]} />
            <meshStandardMaterial color={STEEL} metalness={0.55} roughness={0.45} />
          </mesh>
          <mesh position={[side * 0.29, 0.65, -0.21]} rotation={[0, yaw, 0]}>
            <boxGeometry args={[0.07, 0.055, 0.78]} />
            <meshStandardMaterial
              ref={(m) => {
                barMats.current[1 + i] = m
              }}
              color={AMBER}
              emissive={AMBER}
              emissiveIntensity={0.9}
              metalness={0.3}
              roughness={0.4}
            />
          </mesh>
          <mesh position={[side * 0.58, 0.71, -0.42]}>
            <octahedronGeometry args={[0.045, 0]} />
            <meshStandardMaterial color={AMBER} emissive={AMBER} emissiveIntensity={1.3} />
          </mesh>
        </group>
      ))}
      {/* Materialization ring — hidden except during the one-shot unlock pop */}
      <mesh ref={ringRef} position={[0, 0.02, 0]} rotation={[-Math.PI / 2, 0, 0]} visible={false}>
        <ringGeometry args={[0.9, 1, 56]} />
        <meshBasicMaterial
          ref={ringMat}
          color={CYAN}
          transparent
          opacity={0}
          blending={AdditiveBlending}
          depthWrite={false}
        />
      </mesh>
    </group>
  )
}

/** zone.gamma.relay — hexagonal relay pad west of Beta; walkable via groundHeight. */
function GammaRelay() {
  const unlocked = useGameStore((s) => s.hasGammaRelay)
  const { hexPad, panel } = useMemo(() => getProcTextureKit(), [])
  const torusRef = useRef<Mesh>(null)
  const edgeMats = useRef<(MeshStandardMaterial | null)[]>([])
  const markerMat = useRef<MeshStandardMaterial>(null)

  const bridgeX = (GAMMA_BRIDGE.x0 + GAMMA_BRIDGE.x1) / 2
  const bridgeLen = GAMMA_BRIDGE.x1 - GAMMA_BRIDGE.x0

  useFrame((state, delta) => {
    const t = state.clock.elapsedTime
    if (torusRef.current) torusRef.current.rotation.z += delta * 1.5
    for (let i = 0; i < edgeMats.current.length; i++) {
      const m = edgeMats.current[i]
      if (m) m.emissiveIntensity = 1.0 + Math.sin(t * 2.6 - i * 0.85) * 0.35
    }
    if (markerMat.current) markerMat.current.emissiveIntensity = 0.75 + Math.sin(t * 1.8) * 0.25
  })

  if (!unlocked) return null

  return (
    <group>
      <group position={[GAMMA_CENTER[0], 0, GAMMA_CENTER[1]]}>
        {/* Hex pad — thetaStart 0 = vertex at +X (east toward bridge); matches walk test, top disc, edge bars */}
        <mesh position={[0, 0.05, 0]}>
          <cylinderGeometry args={[GAMMA_RADIUS, GAMMA_RADIUS + 0.3, 0.14, 6, 1, false, 0]} />
          <meshStandardMaterial color="#16283c" metalness={0.35} roughness={0.55} />
        </mesh>
        <mesh position={[0, 0.13, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <circleGeometry args={[GAMMA_RADIUS - 0.3, 6]} />
          <meshStandardMaterial map={hexPad} color="#9fd9d4" metalness={0.25} roughness={0.7} />
        </mesh>
        {/* Edge glow bars along the six hex edges — violet so L3 reads past the cyan L2 annex */}
        {[0, 1, 2, 3, 4, 5].map((k) => {
          const a = Math.PI / 6 + (k * Math.PI) / 3
          const d = GAMMA_RADIUS * 0.8660254
          return (
            <mesh
              key={k}
              position={[d * Math.cos(a), 0.145, d * Math.sin(a)]}
              rotation={[0, -(a + Math.PI / 2), 0]}
            >
              <boxGeometry args={[GAMMA_RADIUS - 0.2, 0.05, 0.07]} />
              <meshStandardMaterial
                ref={(m) => {
                  edgeMats.current[k] = m
                }}
                color={VIOLET}
                emissive={VIOLET}
                emissiveIntensity={1.1}
              />
            </mesh>
          )
        })}
        {/* Center hex marker — same payoff idiom as the Beta pad ring */}
        <mesh position={[0, 0.135, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <ringGeometry args={[0.7, 0.9, 6]} />
          <meshStandardMaterial
            ref={markerMat}
            color={VIOLET}
            emissive={VIOLET}
            emissiveIntensity={0.85}
            transparent
            opacity={0.7}
          />
        </mesh>
        {/* Relay mast — taller than the annex pylon, amber spinner + violet beacon tip */}
        <group position={[0.95, 0, -0.95]}>
          <mesh position={[0, 1.05, 0]}>
            <cylinderGeometry args={[0.05, 0.08, 2.1, 6]} />
            <meshStandardMaterial color={STEEL} metalness={0.55} roughness={0.4} />
          </mesh>
          <mesh ref={torusRef} position={[0, 2.24, 0]} rotation={[Math.PI / 2, 0, 0.2]}>
            <torusGeometry args={[0.3, 0.035, 6, 24]} />
            <meshStandardMaterial color={AMBER} emissive={AMBER} emissiveIntensity={1.2} transparent opacity={0.9} />
          </mesh>
          <mesh position={[0, 2.62, 0]}>
            <octahedronGeometry args={[0.14, 0]} />
            <meshStandardMaterial color={VIOLET} emissive={VIOLET} emissiveIntensity={1.6} />
          </mesh>
        </group>
        {/* Splitter rack — mini Y-bar echoing the L3 blueprint theme */}
        <group position={[-0.95, 0, 0.85]} rotation={[0, 0.5, 0]}>
          {[-0.45, 0.45].map((x) => (
            <mesh key={x} position={[x, 0.275, 0]}>
              <cylinderGeometry args={[0.035, 0.05, 0.55, 6]} />
              <meshStandardMaterial color={STEEL} metalness={0.55} roughness={0.45} />
            </mesh>
          ))}
          <mesh position={[0, 0.55, 0.08]}>
            <boxGeometry args={[0.06, 0.05, 0.42]} />
            <meshStandardMaterial color={CYAN} emissive={CYAN} emissiveIntensity={0.9} />
          </mesh>
          {([[-1, 0.9], [1, -0.9]] as const).map(([side, yaw]) => (
            <mesh key={side} position={[side * 0.14, 0.55, -0.21]} rotation={[0, yaw, 0]}>
              <boxGeometry args={[0.06, 0.05, 0.42]} />
              <meshStandardMaterial color={AMBER} emissive={AMBER} emissiveIntensity={0.9} />
            </mesh>
          ))}
        </group>
        <ZoneLabel text="GAMMA RELAY" color={VIOLET} y={1.7} />
      </group>

      {/* Walkway slab from the Beta west rim to the relay east vertex */}
      <group position={[bridgeX, 0, GAMMA_BRIDGE.z]}>
        <mesh position={[0, 0.06, 0]}>
          <boxGeometry args={[bridgeLen + 0.3, 0.12, GAMMA_BRIDGE.halfWidth * 2]} />
          <meshStandardMaterial map={panel} color="#8fb8b4" metalness={0.4} roughness={0.5} />
        </mesh>
        {[-(GAMMA_BRIDGE.halfWidth - 0.06), GAMMA_BRIDGE.halfWidth - 0.06].map((z) => (
          <mesh key={z} position={[0, 0.13, z]}>
            <boxGeometry args={[bridgeLen + 0.3, 0.03, 0.07]} />
            <meshStandardMaterial color={VIOLET} emissive={VIOLET} emissiveIntensity={0.7} />
          </mesh>
        ))}
      </group>
    </group>
  )
}
